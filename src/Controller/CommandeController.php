...
class CommandeController extends AbstractController
{
    ...
    /**
    * @Route("/displayCommandeFourn", name="displayCommandeFourn" , methods={"GET"})
    */
    public function afficherCommandeF(CommandeRepository $repo): Response
    {
        $commandes = $repo->findAll();
        return $this->render('commande/indexfour.html.twig',['commandes'=>$commandes ]);
    }
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, GetUser $getUser): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $commande->setIdUtilisateur($getUser->Get_User());
            $entityManager->persist($commande);
            $entityManager->flush();

            $this->addFlash('success', 'Commande Ajouter!');
        }
     ...
     */
    public function edit(Request $request, Commande $commande, GetUser $getUser ,EntityManagerInterface $entityManager): Response
    {
        $form = $this->createForm(Commande1Type::class, $commande);
        $form->handleRequest($request);
    ...
    /**
     * @Route("/users/data/download", name="pdfdata")
     */
    public function pdf()
    {
